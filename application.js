...
function renderPosts(container, template, collection){
    ...
    $.each( collection , function( key, val ) {
        if (val.image_url.indexOf('missing.png') > -1) {
            val.post_image = "http://assets.codecloudapp.com/sites/570d369d6e6f643d60030000/image/png/1461352407000/HallifaxLogo.png";
        } else {
...
function renderFeatureItems(){
    ...
    $.each(items, function(i, val){
        $('#feature_' + (i+1)).html('<a href="'+ val.url +'"><img src="'+ val.image_url+'" class="hoverer" alt="' +val.name+ '"></a><h5 class="center_h">'+ val.name +'</h5>')
    })
}
