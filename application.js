    ...
    if (type == "reg_hours") {
        $.each( collection , function( key, val ) {
            ...
            if (val.open_time && val.close_time && val.is_closed == false){
                ...
                // var open_time = moment(val.open_time).tz(getPropertyTimeZone()).format();
                open_time = moment(val.open_time).tz(getPropertyTimeZone()).format("h:mm A");
                // var close_time = moment(val.close_time).tz(getPropertyTimeZone()).format();
                close_time = moment(val.close_time).tz(getPropertyTimeZone()).format("h:mm A");
                val.h = val.open_time + " - " + val.close_time;
            } else {
                "Closed"
            }
